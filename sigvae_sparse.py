...
#                                       ,logging = False)(input_)                            
            logits_x, rk = SparseDecoder(input_dim = h_dim
                                       ,logging = False)(input_)                            
            if i == 0:
...
logits_x_iw, rks = decoder(z_sample_iw, x_dim)
...
log_lik_iw = norm * tf.reduce_mean(adj_orig_tile * tf.log(reconstruct_iw + eps) * pos_weight + (1-adj_orig_tile) * tf.log(1 - reconstruct_iw + eps), axis=[0,1])
...
for epoch in range(training_epochs):
    ...
    _, cost, logitx = sess.run([train_op, loss_iw, logits_x_iw], {x: train_xs, WU: warm_up})
    if epoch % display_step == 0:
        print("Epoch:", '%04d' % epoch, "cost_train=", "{:.9f}".format(cost))
        
    if epoch>100:
        roc_curr, ap_curr = get_roc_score_sp(val_edges, val_edges_false)
        val_roc_score.append(roc_curr)
        
        roc_currt, ap_currt = get_roc_score_sp(test_edges, test_edges_false)
